...
class MultiTimeframeAnalyzer:
    ...
    def _analyze_macd(self, latest: pd.Series, prev: pd.Series, indicator: str) -> Dict:
        ...
        return {
            ...
            "value": hist,
            "bias": bullish and strengthening
        }
    ...
    def _calculate_atr(self, df: pd.DataFrame, period: int = 14) -> np.ndarray:
        ...
        tr = np.concatenate([
            tr1[0:1],
            np.maximum(tr1[1:], np.maximum(tr2, tr3))
        ])
    ...
    def _find_liquidity_levels(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        return {
            ...
            "distance_to_bsl": (buy_side_liquidity - current_price) / current_price if buy_side_liquidity else None,
            "distance_to_ssl": (current_price - sell_side_liquidity) / current_price if sell_side_liquidity else None
        }
    ...
    def _generate_demo_data(
        ...
        df = pd.DataFrame({
            'time': times,
            'open': prices,
            'close': [p + random.uniform(-0.0005, 0.0005) for p in prices],
    ...
    def get_market_summary(self, symbol: str) -> Dict:
        ...
        for tf in self.htf_timeframes:
            ...
            summary["timeframes_analyzed"][tf] = {
                "trend": bias.trend,
