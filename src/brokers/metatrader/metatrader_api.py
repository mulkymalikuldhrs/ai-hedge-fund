...
class MetaTraderMT5API(MetaTraderWebAPI):
    ...
    async def authenticate(self, credentials: Dict = None) -> bool:
        ...
        auth_data = {
            'action': 'auth',
            'account': credentials.get('account', self.account),
    ...
    async def get_account_info(self) -> AccountInfo:
        ...
        return AccountInfo(
            account_id=str(result.get('login', self.account)),
            broker_type=self.broker_type,
            balance=result.get('balance', 0),
            equity=result.get('equity', 0),
            margin=result.get('margin', 0),
            free_margin=result.get('margin_free', 0),
            margin_level=result.get('margin_level', 0),
            leverage=result.get('leverage', 100),
    ...
    async def get_tick(self, ticker: str) -> Tick:
        ...
        return Tick(
            ...
            timestamp=datetime.fromtimestamp(tick_data.get('time', 0))
        )
    ...
    async def get_historical_data(
        ...
        params = {
            'symbol': ticker,
            'timeframe': timeframe_map.get(timeframe, 60),
            'count': count
        }
...
class MetaTraderMT4API(MetaTraderMT5API):
    ...
    async def place_order(self, order: Order) -> Order:
        ...
        order_type_map = {
            OrderType.MARKET: 0,
            OrderType.LIMIT: 2,  # MT4 uses 2 for buy limit
            OrderType.STOP: 3    # MT4 uses 3 for buy stop
        }
        ...
        request_data = {
            ...
            'symbol': order.ticker,
            'type': order_type_map.get(order.order_type, 0),
            'volume': order.quantity,  # MT4 uses lots directly
            'price': order.price,
...
class HedgeFundBrokerManager:
    ...
    async def execute_master_signal(
        self,
        ticker: str,
        side: OrderSide,
        quantity: float,
        stop_loss: float = None,
        take_profit: float = None,
        split_mode: str = "EQUAL"  # EQUAL, WEIGHTED, SINGLE
    ) -> Dict[str, Order]:
        """
        ...
        if split_mode == "SINGLE":
            ...
            order = await self.brokers[primary].place_trade(
                ticker, side, quantity, stop_loss=stop_loss, take_profit=take_profit
            )
