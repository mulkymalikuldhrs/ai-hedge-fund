...
def run_enhanced_multi_agent_analysis(tickers: List[str], context: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    for ticker in tickers:
        ...
        enhanced_analysis = {
            'traditional_signals': strategy_results,
        ...
        if INTEGRATIONS_AVAILABLE and enhanced_autonomous_trader.initialized:
            ...
            autonomous_decision = enhanced_autonomous_trader.generate_autonomous_decision(
                ticker, market_data, portfolio_state
            )
    ...
    return {
        ...
        'results': results,
        'integrations_active': INTEGRATIONS_AVAILABLE,
...
def quantitative_analyst_agent(
    ticker: str,
    prices: List,
    fundamentals: Dict = None,
    market_data: Dict = None
    ...
    return QuantAnalystOutput(
        ...
        reasoning=signal.reasoning,
        momentum_scores={
            "5_day": signal.metadata.get("momentum_5d", 0) if signal.metadata else 0,
            "10_day": signal.metadata.get("momentum_10d", 0) if signal.metadata else 0,
            "20_day": signal.metadata.get("momentum_20d", 0) if signal.metadata else 0
        },
...
def technical_analyst_agent(
    ...
    return TechnicalAnalystOutput(
        ...
        reasoning=signal.reasoning,
        indicators={
            "rsi": signal.metadata.get("rsi", 50) if signal.metadata else 50,
            "macd_hist": signal.metadata.get("macd_hist", 0) if signal.metadata else 0,
            "stochastic": signal.metadata.get("stoch_k", 50) if signal.metadata else 50,
            "price_vs_sma50": signal.metadata.get("price_vs_sma50", 0) if signal.metadata else 0
        },
...
def factor_investor_agent(
    ticker: str,
    prices: List,
    fundamentals: Dict = None,
    market_data: Dict = None
) -> FactorInvestorOutput:
    """
    ...
    return FactorInvestorOutput(
        ...
        reasoning=signal.reasoning,
        factor_scores={
            "value": signal.factors.get("value", 0),
            "momentum": signal.factors.get("momentum", 0),
            "quality": signal.factors.get("quality", 0),
            "volatility": signal.factors.get("volatility", 0)
        },
        composite_score=signal.factors.get("composite", 0),
        factor_exposure={
            "value": "Underweight" if signal.factors.get("value", 0) < 0 else "Overweight",
            "momentum": "Long" if signal.factors.get("momentum", 0) > 0 else "Short",
            "quality": "High" if signal.factors.get("quality", 0) > 0 else "Low",
            "volatility": "Low" if signal.factors.get("volatility", 0) > 0 else "High"
        }
    )
...
def earnings_momentum_agent(
    ticker: str,
    prices: List,
    fundamentals: Dict = None,
    market_data: Dict = None
) -> EarningsMomentumOutput:
    """
    Earnings Momentum Agent
    ...
    return EarningsMomentumOutput(
        signal=signal.signal,
        confidence=int(signal.confidence),
        reasoning=signal.reasoning,
        acceleration=accel,
        earnings_trend="Accelerating" if accel > 0.02 else "Decelerating" if accel < -0.02 else "Stable",
        surprise_proxy=ret_60d
    )
...
def mean_reversion_agent(
    ...
    return MeanReversionOutput(
        ...
        z_score=signal.metadata.get("z_score", 0) if signal.metadata else 0,
        rsi=signal.metadata.get("rsi", 50) if signal.metadata else 50,
        bb_position=signal.metadata.get("bb_position", 0) if signal.metadata else 0,
        mean_price=signal.metadata.get("mean_price", 0) if signal.metadata else 0
    )
...
def run_multi_agent_analysis(
    ...
    with ThreadPoolExecutor(max_workers=6) as executor:
        futures = {
            agent: executor.submit(agent_functions[agent])
            for agent in agent_types if agent in agent_functions
        }
    ...
    return MultiAgentAnalysis(
        ...
        agents_called=agent_types,
        signals=signals,
        aggregated_result={
            "buy_count": buy_count,
            "sell_count": sell_count,
            "hold_count": hold_count,
            "strategy_final": strategy_result.final_signal,
            "strategy_confidence": strategy_result.final_confidence,
            "strategy_consensus": strategy_result.consensus_level
        }
    )
...
ENHANCED_AGENTS = {
    "jim_simmons": {
        ...
        "description": "Statistical pattern recognition and quantitative analysis",
        "function": jim_simmons_agent,
        "weight": 1.5
    },
    "quantitative_analyst": {
        "name": "Quantitative Momentum",
        "description": "Multi-timeframe momentum analysis",
        "function": quantitative_analyst_agent,
        "weight": 1.2
    },
    "technical_analyst": {
    ...
    "factor_investor": {
        ...
        "description": "Value, Momentum, Quality, Volatility factors",
        "function": factor_investor_agent,
        "weight": 1.3
    },
    "earnings_momentum": {
        "name": "Earnings Momentum",
        "description": "Earnings acceleration and trends",
        "function": earnings_momentum_agent,
        "weight": 1.1
    },
    "mean_reversion": {
        "name": "Mean Reversion",
        "description": "Oversold/overbought detection",
        "function": mean_reversion_agent,
        "weight": 1.0
    }
}
