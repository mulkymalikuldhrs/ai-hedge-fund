...
class RiskAnalyzer:
    ...
    def value_at_risk_analysis(self, returns: List[Decimal], confidence_levels: List[Decimal] = None) -> Dict[str, Any]:
        ...
        for confidence in confidence_levels:
            ...
            z_scores = {Decimal("0.01"): Decimal("-2.326"), Decimal("0.05"): Decimal("-1.645"), Decimal("0.10"): Decimal("-1.282")}  # 99% confidence  # 95% confidence  # 90% confidence
    ...
    def _get_asset_class_scenarios(self, asset_class: AssetClass) -> Dict[str, Any]:
        ...
        if asset_class == AssetClass.PRIVATE_EQUITY:
            scenarios.update({"recession_scenario": {"description": "Economic recession impact", "expected_impact": -0.30, "recovery_time": "24_months"}, "credit_crunch": {"description": "Limited exit opportunities", "expected_impact": -0.25, "recovery_time": "18_months"}})  # -30%

        elif asset_class == AssetClass.REAL_ESTATE:
            scenarios.update({"interest_rate_shock": {"description": "300bp rate increase", "expected_impact": -0.20, "recovery_time": "12_months"}, "property_market_crash": {"description": "Property values decline", "expected_impact": -0.35, "recovery_time": "36_months"}})

        elif asset_class == AssetClass.COMMODITIES:
            scenarios.update({"demand_shock": {"description": "Global demand reduction", "expected_impact": -0.40, "recovery_time": "6_months"}, "supply_disruption": {"description": "Supply chain disruption", "expected_impact": 0.25, "recovery_time": "12_months"}})  # Positive for commodity prices

        elif asset_class == AssetClass.HEDGE_FUND:
            scenarios.update({"market_volatility_spike": {"description": "VIX above 40", "expected_impact": -0.15, "recovery_time": "3_months"}, "liquidity_crisis": {"description": "Redemption pressure", "expected_impact": -0.20, "recovery_time": "6_months"}})

        elif asset_class == AssetClass.DIGITAL_ASSETS:
            scenarios.update({"regulatory_crackdown": {"description": "Major regulatory restrictions", "expected_impact": -0.50, "recovery_time": "12_months"}, "crypto_winter": {"description": "Extended bear market", "expected_impact": -0.70, "recovery_time": "24_months"}})
