...
from config import MarketData, CashFlow, Performance, AssetParameters, AssetClass, Constants, Config, InvestmentMethod
...
class PrivateEquityAnalyzer(AlternativeInvestmentBase):
    ...
    def process_capital_call(self, amount: Decimal, call_date: str, description: str = None) -> None:
        ...
        cash_flow = CashFlow(date=call_date, amount=-abs(amount), cf_type="capital_call", description=description or f"Capital call - {call_date}")  # Negative for outflow
    ...
    def valuation_summary(self) -> Dict[str, Any]:
        ...
        valuation = {
            "investment_overview": {"asset_class": self.parameters.asset_class.value, "fund_name": self.parameters.name, "vintage_year": self.vintage_year, "commitment": float(self.commitment) if self.commitment else None},
            "capital_account": {"total_commitment": float(self.commitment) if self.commitment else None, "called_capital": float(self.called_capital), "uncalled_commitment": float(self.commitment - self.called_capital) if self.commitment else None, "distributed_capital": float(self.distributed_capital), "current_nav": float(self.current_nav)},
            "performance_metrics": key_metrics,
            "cash_flow_summary": {"number_of_capital_calls": len([cf for cf in self.cash_flows if cf.cf_type == "capital_call"]), "number_of_distributions": len([cf for cf in self.cash_flows if cf.cf_type == "distribution"]), "total_cash_flows": len(self.cash_flows)},
        }
...
class PrivateDebtAnalyzer(AlternativeInvestmentBase):
    ...
    def credit_risk_assessment(self) -> Dict[str, Any]:
        ...
        risk_factors = {"senior": {"recovery_rate": 0.80, "risk_weight": 1.0}, "mezzanine": {"recovery_rate": 0.50, "risk_weight": 1.5}, "subordinated": {"recovery_rate": 0.20, "risk_weight": 2.0}}
...
class PrivateCapitalPortfolio:
    ...
    def portfolio_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            "portfolio_overview": {"total_nav": float(total_nav), "total_commitments": float(total_commitments), "total_called_capital": float(total_called), "total_distributions": float(total_distributed), "number_pe_investments": len(self.pe_investments), "number_pd_investments": len(self.pd_investments)},
            "pe_portfolio": {"average_irr": float(sum(pe_irrs) / len(pe_irrs)) if pe_irrs else None, "average_moic": float(sum(pe_moics) / len(pe_moics)) if pe_moics else None, "total_pe_nav": float(sum(pe_navs))},
            "pd_portfolio": {"average_yield": float(sum(pd_yields) / len(pd_yields)) if pd_yields else None, "total_pd_nav": float(sum(pd_navs))},
        }
