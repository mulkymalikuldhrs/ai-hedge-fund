...
from config import MarketData, CashFlow, Performance, AssetParameters, AssetClass, Constants, Config, HedgeFundStrategy
...
class HedgeFundAnalyzer(AlternativeInvestmentBase):
    ...
    def valuation_summary(self) -> Dict[str, Any]:
        ...
        return {
            "fund_overview": {"strategy": self.strategy.value, "management_fee": float(self.parameters.management_fee) if self.parameters.management_fee else None, "performance_fee": float(self.parameters.performance_fee) if self.parameters.performance_fee else None, "hurdle_rate": float(self.hurdle_rate), "high_water_mark": float(self.high_water_mark), "leverage": float(self.leverage)},
