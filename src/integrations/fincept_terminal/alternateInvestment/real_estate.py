...
from config import MarketData, CashFlow, Performance, AssetParameters, AssetClass, Constants, Config, RealEstateType
...
class RealEstateAnalyzer(AlternativeInvestmentBase):
    ...
    def valuation_summary(self) -> Dict[str, Any]:
        ...
        return {
            "property_overview": {"property_type": self.property_type.value, "acquisition_price": float(self.acquisition_price) if self.acquisition_price else None, "current_market_value": float(self.current_market_value) if self.current_market_value else None, "gross_rental_income": float(self.gross_rental_income) if self.gross_rental_income else None, "vacancy_rate": float(self.vacancy_rate)},
            "financial_metrics": self.calculate_key_metrics(),
            "valuation_approaches": {"direct_cap": float(self.direct_capitalization_value(self.cap_rate)) if self.cap_rate else None, "dcf": self.dcf_valuation()},
        }
...
class REITAnalyzer(AlternativeInvestmentBase):
    ...
    def valuation_summary(self) -> Dict[str, Any]:
        ...
        return {
            "reit_overview": {"shares_outstanding": float(self.shares_outstanding) if self.shares_outstanding else None, "total_assets": float(self.total_assets) if self.total_assets else None, "total_debt": float(self.total_debt) if self.total_debt else None, "property_value": float(self.property_value) if self.property_value else None},
            "operating_metrics": self.calculate_key_metrics(),
            "valuation_metrics": {"ffo": float(self.calculate_ffo()) if self.calculate_ffo() else None, "affo": float(self.calculate_affo()) if self.calculate_affo() else None, "nav_per_share": float(self.calculate_nav_per_share()) if self.calculate_nav_per_share() else None},
        }
...
class InfrastructureAnalyzer(AlternativeInvestmentBase):
    ...
    def valuation_summary(self) -> Dict[str, Any]:
        ...
        return {
            "infrastructure_overview": {"infrastructure_type": self.infrastructure_type, "regulatory_framework": self.regulatory_framework, "revenue_model": self.revenue_model, "concession_period": self.concession_period, "inflation_indexation": self.inflation_indexation},
...
class RealEstatePortfolio:
    ...
    def portfolio_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            "portfolio_overview": {"total_portfolio_nav": float(total_nav), "direct_real_estate_count": len(self.real_estate_investments), "reit_count": len(self.reit_investments), "infrastructure_count": len(self.infrastructure_investments)},
            "allocation": {"direct_real_estate": {"nav": float(re_total), "weight": float(re_total / total_nav) if total_nav > 0 else 0}, "reits": {"nav": float(reit_total), "weight": float(reit_total / total_nav) if total_nav > 0 else 0}, "infrastructure": {"nav": float(infra_total), "weight": float(infra_total / total_nav) if total_nav > 0 else 0}},
        }
