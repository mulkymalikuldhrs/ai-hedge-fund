...
from .base_models import BaseValuationModel, CompanyData, MarketData, ValuationResult, ValuationMethod, CalculationEngine, ModelValidator, ValidationError
...
def single_stage_ri_valuation(current_book_value: float, roe: float, required_return: float, growth_rate: float, shares_outstanding: float) -> float:
...
def eva_firm_valuation(current_invested_capital: float, projected_evas: List[float], wacc: float, terminal_growth: float = 0) -> float:
