...
class BusinessCycleAnalyzer(EconomicsBase):
    ...
    def _analyze_indicators_by_phase(self, indicators: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "leading_indicators": {
                ...
                "examples": {"stock_market": indicators.get("stock_market_performance", "N/A"), "consumer_confidence": indicators.get("consumer_confidence", "N/A"), "new_business_formation": indicators.get("new_business_starts", "N/A"), "yield_curve": indicators.get("yield_curve_slope", "N/A")},
                "investment_utility": "Most valuable for timing market entry/exit",
            },
            "coincident_indicators": {"description": "Change at the same time as the economy", "examples": {"gdp_growth": indicators.get("gdp_growth_rate", "N/A"), "employment": indicators.get("employment_rate", "N/A"), "industrial_production": indicators.get("industrial_production", "N/A"), "retail_sales": indicators.get("retail_sales_growth", "N/A")}, "investment_utility": "Confirm current economic state"},
            "lagging_indicators": {
                "description": "Change after the economy has changed direction",
                "examples": {"unemployment_rate": indicators.get("unemployment_rate", "N/A"), "inflation_rate": indicators.get("inflation_rate", "N/A"), "interest_rates": indicators.get("interest_rate", "N/A"), "corporate_profits": indicators.get("corporate_profit_growth", "N/A")},
    ...
    def _get_phase_characteristics(self, phase: str) -> Dict[str, Any]:
        ...
        characteristics = {
            ...
            "peak": {"duration": "Brief period (months)", "gdp_growth": "Positive but slowing", "unemployment": "At cyclical lows", "inflation": "At or near cyclical highs", "interest_rates": "At cyclical highs", "business_activity": "Capacity constraints, labor shortages", "consumer_behavior": "High confidence but spending slowing", "financial_markets": "Stock markets vulnerable, tight credit"},
            "contraction": {
    ...
    def _get_investment_implications(self, phase: str) -> Dict[str, Any]:
        ...
        implications = {
            "expansion": {"equity_strategy": "Favor cyclical stocks, growth sectors", "fixed_income": "Shorter duration, higher yield focus", "sectors_to_favor": ["Technology", "Consumer Discretionary", "Industrials"], "sectors_to_avoid": ["Utilities", "Consumer Staples"], "overall_risk": "Moderate to High"},
            "peak": {"equity_strategy": "Defensive positioning, quality focus", "fixed_income": "Extend duration, prepare for rate cuts", "sectors_to_favor": ["Healthcare", "Consumer Staples", "Utilities"], "sectors_to_avoid": ["Cyclicals", "Small caps"], "overall_risk": "Reduce risk exposure"},
            "contraction": {"equity_strategy": "Defensive stocks, dividend focus", "fixed_income": "High quality bonds, government securities", "sectors_to_favor": ["Consumer Staples", "Healthcare", "Utilities"], "sectors_to_avoid": ["Financials", "Materials", "Energy"], "overall_risk": "Low risk, capital preservation"},
            "trough": {"equity_strategy": "Prepare for cyclical recovery, value opportunities", "fixed_income": "Shorten duration, prepare for rate rises", "sectors_to_favor": ["Financials", "Technology", "Industrials"], "sectors_to_avoid": ["Defensive sectors becoming expensive"], "overall_risk": "Gradually increase risk"},
        }
...
class CreditCycleAnalyzer(EconomicsBase):
    ...
    def analyze_credit_cycle(self, credit_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "risk_assessment": self._assess_credit_risks(credit_growth, leverage_ratio, default_rates, asset_prices),
    ...
    def _get_credit_phase_characteristics(self, phase: str) -> Dict[str, Any]:
        ...
        characteristics = {
            "expansion": {"credit_growth": "Accelerating", "lending_standards": "Loosening", "credit_spreads": "Tightening", "default_rates": "Low and declining", "asset_prices": "Rising", "risk_appetite": "Increasing", "typical_duration": "3-7 years"},
            "peak": {"credit_growth": "Very high but potentially slowing", "lending_standards": "Very loose", "credit_spreads": "Very tight", "default_rates": "Near cyclical lows", "asset_prices": "Near peaks, potential bubbles", "risk_appetite": "Excessive", "typical_duration": "6-18 months"},
            "contraction": {"credit_growth": "Negative", "lending_standards": "Tightening rapidly", "credit_spreads": "Widening", "default_rates": "Rising sharply", "asset_prices": "Declining", "risk_appetite": "Risk aversion", "typical_duration": "1-3 years"},
            "trough": {"credit_growth": "Negative but stabilizing", "lending_standards": "Very tight", "credit_spreads": "Wide but stabilizing", "default_rates": "High but peaking", "asset_prices": "Depressed but stabilizing", "risk_appetite": "Extremely low", "typical_duration": "6-18 months"},
        }
    ...
    def _analyze_financial_stability(self, credit_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "banking_sector_health": {"capital_adequacy": credit_data.get("bank_capital_ratio", "N/A"), "loan_loss_provisions": credit_data.get("loan_loss_provisions", "N/A"), "profitability": credit_data.get("bank_roe", "N/A"), "asset_quality": credit_data.get("npl_ratio", "N/A")},
            "household_sector": {"debt_to_income": credit_data.get("household_debt_ratio", "N/A"), "mortgage_defaults": credit_data.get("mortgage_default_rate", "N/A"), "savings_rate": credit_data.get("household_savings_rate", "N/A")},
            "corporate_sector": {"corporate_debt_ratio": credit_data.get("corporate_debt_gdp", "N/A"), "interest_coverage": credit_data.get("interest_coverage_ratio", "N/A"), "bankruptcy_rate": credit_data.get("corporate_bankruptcy_rate", "N/A")},
            "government_sector": {"debt_to_gdp": credit_data.get("government_debt_gdp", "N/A"), "deficit_ratio": credit_data.get("budget_deficit_gdp", "N/A")},
        }
    ...
    def _credit_cycle_investment_implications(self, phase: str) -> Dict[str, Any]:
        ...
        implications = {
            "expansion": {"credit_sensitive_sectors": "Favor banks, real estate, consumer finance", "fixed_income": "Corporate bonds outperform, credit spreads tighten", "equity_strategy": "Growth and cyclical stocks perform well", "risk_management": "Monitor leverage, prepare for cycle turn"},
            "peak": {"credit_sensitive_sectors": "Begin reducing exposure to credit cyclicals", "fixed_income": "Lock in credit spreads, extend duration", "equity_strategy": "Rotate to defensive sectors", "risk_management": "Reduce overall risk exposure"},
            "contraction": {"credit_sensitive_sectors": "Avoid banks, real estate, high-yield bonds", "fixed_income": "Government bonds, high-grade corporates", "equity_strategy": "Defensive sectors, dividend stocks", "risk_management": "Capital preservation focus"},
            "trough": {"credit_sensitive_sectors": "Prepare for opportunistic investments", "fixed_income": "Distressed debt opportunities", "equity_strategy": "Value opportunities in beaten-down sectors", "risk_management": "Begin rebuilding risk exposure"},
        }
    ...
    def _credit_cycle_policy_implications(self, phase: str, credit_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        base_implications = {
            "expansion": {"monetary_policy": "Consider gradual tightening to prevent bubbles", "macroprudential": "Implement countercyclical capital buffers", "regulatory": "Monitor systemic risk buildup"},
            "peak": {"monetary_policy": "Careful balancing to prevent hard landing", "macroprudential": "Activate countercyclical buffers", "regulatory": "Stress test financial institutions"},
            "contraction": {"monetary_policy": "Aggressive easing to support credit flow", "macroprudential": "Release countercyclical buffers", "regulatory": "Temporary forbearance measures"},
            "trough": {"monetary_policy": "Maintain accommodative stance", "macroprudential": "Gradual rebuilding of buffers", "regulatory": "Support credit intermediation"},
        }
...
class MarketStructureAnalyzer(EconomicsBase):
    ...
    def _get_structure_characteristics(self, structure_type: str) -> Dict[str, Any]:
        ...
        characteristics = {
            "perfect_competition": {"number_of_firms": "Many (hundreds or thousands)", "product_differentiation": "None (homogeneous products)", "barriers_to_entry": "None", "pricing_power": "None (price takers)", "long_run_profits": "Zero economic profits", "efficiency": "Allocatively and productively efficient", "examples": "Agricultural markets, commodity markets"},
            "monopolistic_competition": {"number_of_firms": "Many (dozens to hundreds)", "product_differentiation": "Some (differentiated products)", "barriers_to_entry": "Low", "pricing_power": "Limited (some control over price)", "long_run_profits": "Zero economic profits", "efficiency": "Not fully efficient due to excess capacity", "examples": "Restaurants, retail clothing, personal services"},
            "oligopoly": {"number_of_firms": "Few (typically 3-10)", "product_differentiation": "May be homogeneous or differentiated", "barriers_to_entry": "High", "pricing_power": "Significant (price makers)", "long_run_profits": "Positive economic profits possible", "efficiency": "Generally inefficient, potential for collusion", "examples": "Airlines, telecommunications, automobiles"},
            "monopoly": {"number_of_firms": "One", "product_differentiation": "Unique product (no close substitutes)", "barriers_to_entry": "Very high or absolute", "pricing_power": "Maximum (price maker)", "long_run_profits": "Positive economic profits", "efficiency": "Allocatively inefficient, may be productively efficient", "examples": "Public utilities, patented drugs, natural monopolies"},
        }
    ...
    def _analyze_competitive_dynamics(self, structure_type: str, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        dynamics = {
            "perfect_competition": {"competition_intensity": "Maximum", "strategic_behavior": "None (price takers)", "product_strategy": "Focus on cost efficiency", "market_response": "Immediate price adjustments", "long_term_strategy": "Operational excellence"},
            "monopolistic_competition": {"competition_intensity": "High but differentiated", "strategic_behavior": "Product differentiation focus", "product_strategy": "Brand building, innovation", "market_response": "Gradual price and product adjustments", "long_term_strategy": "Sustainable differentiation"},
            "oligopoly": {"competition_intensity": "Strategic interdependence", "strategic_behavior": "Game theory applies, potential collusion", "product_strategy": "Innovation and differentiation", "market_response": "Strategic reactions to competitors", "long_term_strategy": "Market share protection"},
            "monopoly": {"competition_intensity": "None or minimal", "strategic_behavior": "Price and output optimization", "product_strategy": "Innovation optional", "market_response": "Independent pricing decisions", "long_term_strategy": "Barrier maintenance"},
        }
    ...
    def calculate_breakeven_shutdown_points(self, cost_data: Dict[str, Any], market_structure: str) -> Dict[str, Any]:
        ...
        return {
            "cost_structure": {"fixed_costs": fixed_costs, "variable_cost_per_unit": variable_cost_per_unit, "average_variable_cost": average_variable_cost, "marginal_cost": marginal_cost},
            "breakeven_analysis": {"breakeven_quantity": breakeven_quantity, "breakeven_revenue": breakeven_quantity * market_price if breakeven_quantity else None, "contribution_margin": market_price - variable_cost_per_unit, "contribution_margin_ratio": ((market_price - variable_cost_per_unit) / market_price * self.to_decimal(100)) if market_price > 0 else self.to_decimal(0)},
            "shutdown_analysis": {"shutdown_price": shutdown_price, "current_price": market_price, "should_shutdown": market_price < shutdown_price, "shutdown_loss": fixed_costs if market_price < shutdown_price else self.to_decimal(0)},
            "optimal_production": {"optimal_quantity": optimal_output, "total_revenue": optimal_output * market_price, "total_costs": total_costs(optimal_output), "economic_profit": optimal_output * market_price - total_costs(optimal_output)},
    ...
    def _analyze_pricing_behavior(self, structure_type: str, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        pricing_behavior = {
            "perfect_competition": {"pricing_strategy": "Price taking (no control)", "price_setting": "Market determined", "demand_curve": "Perfectly elastic", "profit_maximization": "P = MC", "price_discrimination": "Not possible"},
            "monopolistic_competition": {"pricing_strategy": "Limited pricing power", "price_setting": "Some control over price", "demand_curve": "Downward sloping but elastic", "profit_maximization": "MR = MC", "price_discrimination": "Limited opportunities"},
            "oligopoly": {"pricing_strategy": "Strategic pricing", "price_setting": "Mutual interdependence", "demand_curve": "Kinked demand curve possible", "profit_maximization": "Game theory considerations", "price_discrimination": "Possible with market segmentation"},
            "monopoly": {"pricing_strategy": "Price maker", "price_setting": "Full control subject to demand", "demand_curve": "Downward sloping", "profit_maximization": "MR = MC", "price_discrimination": "Multiple types possible"},
        }
    ...
    def _analyze_efficiency_implications(self, structure_type: str) -> Dict[str, Any]:
        ...
        efficiency = {
            "perfect_competition": {"allocative_efficiency": "Yes (P = MC)", "productive_efficiency": "Yes (minimum ATC)", "dynamic_efficiency": "Limited (low profits for R&D)", "consumer_surplus": "Maximized", "deadweight_loss": "None"},
            "monopolistic_competition": {"allocative_efficiency": "No (P > MC)", "productive_efficiency": "No (excess capacity)", "dynamic_efficiency": "Moderate (innovation incentives)", "consumer_surplus": "Reduced due to higher prices", "deadweight_loss": "Small"},
            "oligopoly": {"allocative_efficiency": "No (P > MC)", "productive_efficiency": "Uncertain (may achieve scale economies)", "dynamic_efficiency": "High (R&D competition)", "consumer_surplus": "Significantly reduced", "deadweight_loss": "Moderate to large"},
            "monopoly": {"allocative_efficiency": "No (P > MC)", "productive_efficiency": "Uncertain (may achieve scale economies)", "dynamic_efficiency": "Low (limited competition pressure)", "consumer_surplus": "Minimized", "deadweight_loss": "Large"},
        }
    ...
    def _get_regulatory_considerations(self, structure_type: str) -> Dict[str, Any]:
        ...
        regulations = {
            "perfect_competition": {"antitrust_concern": "None", "regulation_needed": "Minimal", "focus": "Maintain competitive conditions", "interventions": "Prevent artificial barriers to entry"},
            "monopolistic_competition": {"antitrust_concern": "Low", "regulation_needed": "Light touch", "focus": "Consumer protection, fair advertising", "interventions": "Truth in advertising, quality standards"},
            "oligopoly": {"antitrust_concern": "High", "regulation_needed": "Active monitoring", "focus": "Prevent collusion, monitor mergers", "interventions": "Merger review, price fixing prevention"},
            "monopoly": {"antitrust_concern": "Very high", "regulation_needed": "Heavy regulation or breakup", "focus": "Price regulation, service quality", "interventions": "Rate regulation, structural remedies"},
        }
