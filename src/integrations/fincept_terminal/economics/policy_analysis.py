...
class FiscalPolicyAnalyzer(EconomicsBase):
    ...
    def compare_fiscal_monetary(self) -> Dict[str, Any]:
        ...
        return {
            "fiscal_policy": {"authority": "Government (legislative/executive)", "tools": ["Government spending", "Taxation", "Transfer payments"], "targets": ["Economic growth", "Employment", "Income distribution"], "transmission": "Direct impact on aggregate demand", "lag_time": "Long (6-18 months)", "political_influence": "High", "flexibility": "Low (requires legislative approval)"},
            "monetary_policy": {"authority": "Central bank", "tools": ["Interest rates", "Money supply", "Reserve requirements"], "targets": ["Price stability", "Economic growth", "Financial stability"], "transmission": "Indirect through financial markets", "lag_time": "Medium (3-12 months)", "political_influence": "Low (independent)", "flexibility": "High (quick implementation)"},
            "interaction_effects": {"complementary": "Both expansionary during recession", "conflicting": "Fiscal expansion with monetary tightening", "coordination_importance": "Critical for policy effectiveness"},
        }

    def analyze_fiscal_tools(self, policy_data: Dict[str, Any]) -> Dict[str, Any]:
        """Analyze fiscal policy tools and their effects"""
        tools_analysis = {
            "government_spending": {"multiplier_effect": self._calculate_spending_multiplier(policy_data), "advantages": ["Direct job creation", "Infrastructure investment", "Quick stimulus"], "disadvantages": ["Crowding out private investment", "Debt accumulation", "Political interference"], "effectiveness": "High during recessions, moderate during expansions"},
            "taxation": {"multiplier_effect": self._calculate_tax_multiplier(policy_data), "advantages": ["Broad-based impact", "Revenue generation", "Incentive alignment"], "disadvantages": ["Lagged response", "Political constraints", "Distortionary effects"], "effectiveness": "Moderate, depends on tax type and economic conditions"},
            "transfer_payments": {"multiplier_effect": self._calculate_transfer_multiplier(policy_data), "advantages": ["Targeted support", "Automatic stabilizers", "Social safety net"], "disadvantages": ["Potential dependency", "Fiscal burden", "Limited growth impact"], "effectiveness": "High for consumption support, moderate for growth"},
        }
    ...
    def assess_debt_sustainability(self, debt_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "debt_dynamics": {"interest_burden": interest_rate * debt_gdp / self.to_decimal(100), "growth_benefit": gdp_growth * debt_gdp / self.to_decimal(100), "primary_contribution": primary_balance},
    ...
    def _assess_stance_impact(self, stance: str, impulse: Decimal) -> Dict[str, str]:
        ...
        impacts = {
            "Expansionary": {"gdp_impact": "Positive stimulus to growth", "employment_impact": "Job creation likely", "inflation_risk": "Potential upward pressure", "debt_impact": "Increased deficit spending"},
            "Contractionary": {"gdp_impact": "Negative drag on growth", "employment_impact": "Potential job losses", "inflation_risk": "Reduced price pressures", "debt_impact": "Deficit reduction"},
            "Neutral": {"gdp_impact": "Minimal direct impact", "employment_impact": "Status quo maintained", "inflation_risk": "No significant pressure", "debt_impact": "Stable debt dynamics"},
        }
...
class MonetaryPolicyAnalyzer(EconomicsBase):
    ...
    def analyze_central_bank_roles(self) -> Dict[str, Any]:
        ...
        return {
            "primary_objectives": {"price_stability": "Maintain low and stable inflation", "economic_growth": "Support sustainable economic expansion", "financial_stability": "Ensure stable financial system", "employment": "Some central banks have explicit employment mandate"},
            "key_functions": {"monetary_policy": "Set interest rates and control money supply", "banking_supervision": "Regulate and supervise financial institutions", "lender_of_last_resort": "Provide emergency liquidity to banks", "currency_issuance": "Issue and manage national currency", "government_banker": "Provide banking services to government"},
            "independence_importance": {"political_independence": "Avoid short-term political pressures", "operational_independence": "Freedom to choose policy tools", "accountability": "Report to legislature on performance"},
        }
    ...
    def analyze_monetary_tools(self, policy_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "conventional_tools": {
                "policy_rate": {"description": "Central bank key interest rate", "current_rate": policy_data.get("policy_rate", "N/A"), "transmission": "Affects all market rates", "effectiveness": "High when rates above zero lower bound"},
                "reserve_requirements": {"description": "Banks required reserve ratio", "current_ratio": policy_data.get("reserve_ratio", "N/A"), "transmission": "Affects bank lending capacity", "effectiveness": "Powerful but rarely used"},
                "open_market_operations": {"description": "Buy/sell government securities", "current_balance_sheet": policy_data.get("central_bank_balance_sheet", "N/A"), "transmission": "Direct impact on money supply", "effectiveness": "Most frequently used tool"},
            },
            "unconventional_tools": {"quantitative_easing": "Large-scale asset purchases", "forward_guidance": "Communication about future policy", "negative_rates": "Below-zero policy rates", "yield_curve_control": "Target specific maturity yields"},
    ...
    def analyze_targeting_strategies(self, strategy_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        strategies = {
            "inflation_targeting": {"target": strategy_data.get("inflation_target", "2%"), "advantages": ["Clear communication", "Credible commitment", "Flexible response"], "disadvantages": ["Ignores other variables", "May miss asset bubbles", "Measurement issues"], "effectiveness": "High for anchoring expectations"},
            "interest_rate_targeting": {"target": strategy_data.get("interest_rate_target", "Variable"), "advantages": ["Direct control", "Clear signal", "Quick transmission"], "disadvantages": ["May ignore inflation", "Procyclical risks", "Zero lower bound"], "effectiveness": "High for short-term stabilization"},
            "exchange_rate_targeting": {"target": strategy_data.get("exchange_rate_target", "N/A"), "advantages": ["Trade stability", "Import price stability", "Simple communication"], "disadvantages": ["Loss of monetary independence", "Vulnerable to attacks", "Limited flexibility"], "effectiveness": "Moderate, depends on economic structure"},
        }
    ...
    def assess_policy_effectiveness(self, effectiveness_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "effectiveness_factors": {"central_bank_credibility": effectiveness_data.get("credibility_index", "N/A"), "financial_system_development": effectiveness_data.get("financial_development_index", "N/A"), "economic_structure": effectiveness_data.get("economic_structure", "N/A"), "inflation_expectations_anchoring": effectiveness_data.get("expectations_anchored", "N/A")},
            "policy_limitations": {"zero_lower_bound": "Cannot cut rates below certain level", "liquidity_trap": "Money demand becomes perfectly elastic", "long_and_variable_lags": "Policy effects take 6-18 months", "asset_bubbles": "Difficulty identifying and responding to bubbles", "financial_stability": "Trade-offs between price and financial stability"},
    ...
    def analyze_policy_interaction(self, interaction_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        interaction_matrix = {
            ("expansionary", "expansionary"): {"coordination": "Aligned", "economic_impact": "Strong stimulus", "risks": "Overheating, inflation", "appropriate_when": "Deep recession"},
            ("expansionary", "contractionary"): {"coordination": "Conflicting", "economic_impact": "Uncertain, depends on relative strength", "risks": "Policy ineffectiveness", "appropriate_when": "Fiscal stimulus with inflation concerns"},
            ("contractionary", "expansionary"): {"coordination": "Conflicting", "economic_impact": "Uncertain, mixed signals", "risks": "Policy confusion", "appropriate_when": "Fiscal consolidation with growth support"},
            ("contractionary", "contractionary"): {"coordination": "Aligned", "economic_impact": "Strong contraction", "risks": "Excessive slowdown", "appropriate_when": "High inflation, overheating"},
        }
    ...
    def _analyze_transmission_mechanism(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "interest_rate_channel": {"mechanism": "Policy rate → Market rates → Investment/Consumption", "strength": "Strong in developed economies", "lag": "6-12 months"},
            "credit_channel": {"mechanism": "Policy → Bank lending → Economic activity", "strength": "Important for bank-dependent economies", "lag": "3-9 months"},
            "exchange_rate_channel": {"mechanism": "Policy rate → Exchange rate → Net exports", "strength": "Strong in open economies", "lag": "3-6 months"},
            "asset_price_channel": {"mechanism": "Policy → Asset prices → Wealth → Consumption", "strength": "Important with developed capital markets", "lag": "6-18 months"},
            "expectations_channel": {"mechanism": "Policy communication → Expectations → Decisions", "strength": "Critical for all economies", "lag": "Immediate to 3 months"},
        }
...
class CentralBankAnalyzer(EconomicsBase):
    ...
    def assess_central_bank_quality(self, cb_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        quality_metrics = {
            "independence": {"score": self.to_decimal(cb_data.get("independence_index", 0.5)), "components": ["Political independence", "Operational independence", "Financial independence"], "importance": "Critical for credibility and long-term focus"},
            "transparency": {"score": self.to_decimal(cb_data.get("transparency_index", 0.5)), "components": ["Clear communication", "Regular reporting", "Decision explanations"], "importance": "Essential for expectation management"},
            "accountability": {"score": self.to_decimal(cb_data.get("accountability_index", 0.5)), "components": ["Legislative oversight", "Performance reporting", "Public scrutiny"], "importance": "Democratic legitimacy and performance monitoring"},
            "technical_competence": {"score": self.to_decimal(cb_data.get("competence_index", 0.5)), "components": ["Staff expertise", "Research capability", "Analysis quality"], "importance": "Effective policy design and implementation"},
        }
