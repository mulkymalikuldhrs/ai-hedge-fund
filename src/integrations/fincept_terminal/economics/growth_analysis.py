...
class GrowthAnalyzer(EconomicsBase):
    ...
    def _analyze_developed_economy_factors(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        favoring_factors = {
            "technological_innovation": {"score": rd_spending * self.to_decimal(10), "description": "High R&D spending drives innovation-led growth", "weight": self.to_decimal(0.25)},  # R&D as % of GDP scaled
            "human_capital": {"score": education_index * self.to_decimal(100), "description": "Skilled workforce enables productivity gains", "weight": self.to_decimal(0.20)},
            "institutional_quality": {"score": infrastructure_quality, "description": "Strong institutions support efficient markets", "weight": self.to_decimal(0.20)},
            "capital_deepening": {"score": self.to_decimal(85), "description": "Existing capital stock supports productivity", "weight": self.to_decimal(0.15)},  # Typically high in developed economies
        }

        limiting_factors = {
            "demographic_constraints": {"score": aging_ratio, "description": "Aging population reduces labor force growth", "weight": self.to_decimal(0.30)},
            "diminishing_returns": {"score": gdp_per_capita / self.to_decimal(1000), "description": "High income levels face diminishing marginal returns", "weight": self.to_decimal(0.25)},  # Higher GDP per capita = more diminishing returns
            "low_population_growth": {"score": max(self.to_decimal(0), self.to_decimal(2) - population_growth) * self.to_decimal(50), "description": "Low population growth limits labor force expansion", "weight": self.to_decimal(0.20)},
            "mature_economy_constraints": {"score": self.to_decimal(70), "description": "Limited catch-up growth opportunities", "weight": self.to_decimal(0.25)},  # Fixed score for developed economies
        }
    ...
    def _analyze_developing_economy_factors(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        favoring_factors = {
            "catch_up_potential": {"score": max(self.to_decimal(0), self.to_decimal(50) - gdp_per_capita / self.to_decimal(1000)), "description": "Low income levels allow rapid catch-up growth", "weight": self.to_decimal(0.25)},
            "demographic_dividend": {"score": min(population_growth * self.to_decimal(25), self.to_decimal(100)), "description": "Young population provides growing workforce", "weight": self.to_decimal(0.20)},
            "capital_accumulation": {"score": savings_rate * self.to_decimal(2), "description": "High savings enable capital investment", "weight": self.to_decimal(0.20)},
            "technology_transfer": {"score": fdi_inflows * self.to_decimal(10), "description": "FDI brings advanced technology and knowledge", "weight": self.to_decimal(0.15)},
            "education_expansion": {"score": education_enrollment, "description": "Growing human capital base", "weight": self.to_decimal(0.20)},
        }

        limiting_factors = {
            "institutional_weaknesses": {"score": self.to_decimal(100) - institutional_quality, "description": "Weak institutions hinder efficient resource allocation", "weight": self.to_decimal(0.30)},
            "infrastructure_gaps": {"score": self.to_decimal(80), "description": "Inadequate infrastructure limits productivity", "weight": self.to_decimal(0.25)},  # Typically high in developing countries
            "human_capital_deficits": {"score": self.to_decimal(100) - education_enrollment, "description": "Limited education reduces productivity potential", "weight": self.to_decimal(0.20)},
            "external_dependence": {"score": self.to_decimal(60), "description": "Dependence on external financing and technology", "weight": self.to_decimal(0.25)},  # Moderate score for most developing economies
        }
    ...
    def analyze_stock_market_growth_relationship(self, market_data: Dict[str, Any], economic_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "return_decomposition": {"earnings_growth": earnings_growth_component, "dividend_yield": dividend_component, "valuation_change": valuation_change_component},
            "long_run_relationship": {"description": "In long run, stock returns should converge to sustainable growth + dividend yield", "theoretical_return": sustainable_growth + dividend_yield, "current_deviation": avg_stock_return - (sustainable_growth + dividend_yield), "sustainable": abs(excess_return) < self.to_decimal(2)},  # Within 2% considered sustainable
    ...
    def _equity_investor_implications(self, output_gap: Decimal, potential_growth: Decimal, gdp_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "earnings_growth_potential": {"description": "Potential GDP growth sets upper bound for long-term earnings growth", "implication": f"Long-term earnings growth limited to ~{potential_growth:.1f}% annually", "current_position": "Above potential" if output_gap > 0 else "Below potential"},
            "cyclical_positioning": {"output_gap": output_gap, "interpretation": self._interpret_output_gap_equity(output_gap), "strategy": self._equity_strategy_from_gap(output_gap)},
            "sector_implications": {"cyclical_sectors": "Sensitive to output gap fluctuations", "defensive_sectors": "Less sensitive, focus on long-term potential growth", "growth_sectors": "Beneficiaries of productivity improvements driving potential growth"},
            "valuation_framework": {"sustainable_pe": f"Long-term PE ratios should reflect potential growth of {potential_growth:.1f}%", "cyclical_adjustment": "Adjust for temporary deviations from potential"},
        }
    ...
    def _fixed_income_implications(self, output_gap: Decimal, potential_growth: Decimal, gdp_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "monetary_policy_stance": {"output_gap": output_gap, "policy_implication": self._monetary_policy_from_gap(output_gap), "interest_rate_direction": self._interest_rate_direction(output_gap)},
            "inflation_expectations": {"gap_pressure": "Positive gap = inflationary pressure" if output_gap > 0 else "Negative gap = disinflationary pressure", "long_term_anchor": f"Long-term inflation should align with potential growth of {potential_growth:.1f}%", "current_risk": "Inflation risk elevated" if output_gap > self.to_decimal(2) else "Inflation risk contained"},
            "yield_curve_implications": {"short_end": "Driven by central bank response to output gap", "long_end": "Anchored by potential growth and inflation expectations", "curve_shape": self._yield_curve_shape(output_gap)},
            "credit_risk_assessment": {"corporate_earnings": "Tied to actual vs potential GDP performance", "default_risk": "Lower when economy operates near potential", "recovery_rates": "Higher potential growth supports better recovery values"},
        }
    ...
    def forecast_potential_gdp(self, historical_data: Dict[str, Any], forecast_assumptions: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "growth_accounting_framework": {"formula": "GDP Growth = Productivity Growth + α×Capital Growth + (1-α)×Labor Growth", "capital_share_alpha": alpha, "labor_share": self.to_decimal(1) - alpha},
            "historical_analysis": {"historical_potential_growth": historical_potential, "average_historical_potential": sum(historical_potential) / self.to_decimal(len(historical_potential)) if historical_potential else self.to_decimal(0), "trend_components": {"productivity": trend_productivity, "labor_force": trend_labor, "capital_stock": trend_capital}},
            "forecast": {"periods": forecast_periods, "potential_gdp_growth": forecast_potential_growth, "components": {"productivity_contribution": productivity_growth_forecast, "capital_contribution": alpha * capital_growth_forecast, "labor_contribution": (self.to_decimal(1) - alpha) * labor_growth_forecast}, "assumptions": forecast_assumptions},
    ...
    def _sensitivity_analysis_potential_gdp(self, alpha: Decimal, prod_growth: Decimal, cap_growth: Decimal, lab_growth: Decimal) -> Dict[str, Any]:
        ...
        # Test scenarios
        scenarios = {"productivity_high": prod_growth + self.to_decimal(0.005), "productivity_low": prod_growth - self.to_decimal(0.005), "capital_high": cap_growth + self.to_decimal(0.01), "capital_low": cap_growth - self.to_decimal(0.01), "labor_high": lab_growth + self.to_decimal(0.005), "labor_low": lab_growth - self.to_decimal(0.005)}  # +0.5pp  # -0.5pp  # +1pp  # -1pp  # +0.5pp  # -0.5pp
    ...
    def calculate(self, analysis_type: str, **kwargs) -> Dict[str, Any]:
        ...
        calculations = {
            "growth_factors": lambda: self.compare_growth_factors(kwargs["country_type"], kwargs["economic_data"]),
            "stock_growth_relationship": lambda: self.analyze_stock_market_growth_relationship(kwargs["market_data"], kwargs["economic_data"]),
            "potential_gdp_importance": lambda: self.potential_gdp_importance(kwargs["gdp_data"], kwargs.get("investor_type", "both")),
            "forecast_potential_gdp": lambda: self.forecast_potential_gdp(kwargs["historical_data"], kwargs["forecast_assumptions"]),
        }
...
class ProductivityAnalyzer(EconomicsBase):
    ...
    def analyze_capital_deepening_vs_technology(self, productivity_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "decomposition": {"labor_productivity_growth": labor_productivity, "capital_deepening_contribution": capital_deepening_contribution, "technology_contribution": technology_contribution, "residual": residual},
            "relative_importance": {"capital_deepening_share": (capital_deepening_contribution / labor_productivity * self.to_decimal(100)) if labor_productivity != 0 else self.to_decimal(0), "technology_share": (technology_contribution / labor_productivity * self.to_decimal(100)) if labor_productivity != 0 else self.to_decimal(0)},
            "economic_implications": {
                "capital_deepening": {"description": "Increasing capital per worker", "effects": "Diminishing returns, temporary boost to productivity", "sustainability": "Limited by diminishing marginal returns", "policy_focus": "Investment incentives, savings rates"},
                "technological_progress": {"description": "Improvements in total factor productivity", "effects": "Sustainable productivity gains, no diminishing returns", "sustainability": "Can sustain long-term growth", "policy_focus": "R&D investment, education, innovation"},
            },
            "growth_sustainability": self._assess_growth_sustainability(capital_deepening_contribution, technology_contribution),
        }
...
class ConvergenceAnalyzer(EconomicsBase):
    ...
    def _explain_convergence_theories(self) -> Dict[str, Any]:
        ...
        return {
            "neoclassical_theory": {"prediction": "Unconditional convergence due to diminishing returns", "mechanism": "Poor countries have higher marginal returns to capital", "assumptions": "Same technology, preferences, institutions", "reality": "Limited empirical support for unconditional convergence"},
            "conditional_convergence": {"prediction": "Convergence to country-specific steady states", "mechanism": "Countries converge to own equilibrium based on fundamentals", "factors": "Savings rates, population growth, technology, institutions", "evidence": "Stronger empirical support"},
            "endogenous_growth": {"prediction": "Divergence possible due to increasing returns", "mechanism": "Knowledge spillovers, human capital externalities", "implications": "Rich countries may grow faster permanently", "policy": "Government intervention may be needed"},
        }
...
class DemographicAnalyzer(EconomicsBase):
    ...
    def analyze_demographic_impact(self, demographic_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "demographic_dividend_analysis": self._analyze_demographic_dividend(working_age_share, dependency_ratio, population_growth),
            "immigration_impact": self._analyze_immigration_impact(immigration_rate, immigrant_age_profile, labor_force_participation),
            "labor_force_dynamics": self._analyze_labor_force_participation(labor_force_participation, female_participation, working_age_share),
            "long_term_sustainability": self._assess_demographic_sustainability(fertility_rate, life_expectancy, dependency_ratio),
    ...
    def _analyze_labor_force_participation(self, overall_lfpr: Decimal, female_lfpr: Decimal, working_age_share: Decimal) -> Dict[str, Any]:
        ...
        return {
            ...
            "policy_levers": ["Childcare support to increase female participation", "Flexible work arrangements", "Education and skills training", "Retirement age adjustments for aging societies"],
        }
