...
class SpotForwardAnalyzer(EconomicsBase):
    ...
    def calculate(self, calculation_type: str, **kwargs) -> Dict[str, Any]:
        ...
        calculations = {
            "bid_offer_spread": lambda: self.calculate_bid_offer_spread(self.to_decimal(kwargs["bid"]), self.to_decimal(kwargs["ask"])),
            "forward_premium_discount": lambda: self.calculate_forward_premium_discount(self.to_decimal(kwargs["spot_rate"]), self.to_decimal(kwargs["forward_rate"]), self.to_decimal(kwargs["time_to_maturity"])),
            "mark_to_market": lambda: self.mark_to_market_forward(kwargs["contract_details"], kwargs["current_market_data"]),
            "spread_factors": lambda: self.factors_affecting_spread(kwargs["currency_pair"], kwargs["market_conditions"]),
        }
