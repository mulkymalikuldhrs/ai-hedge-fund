...
from base.base_provider import BacktestingProviderBase, BacktestResult, PerformanceMetrics, Trade, EquityPoint, BacktestStatistics, json_response, parse_json_input
...
class FastTradeProvider(BacktestingProviderBase):
    ...
    def run_backtest(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            ft_config = self._build_fasttrade_config(strategy_def, parameters, initial_capital, data)
            ...
            backtest_result = self._convert_result(result, request.get("id", self._generate_id()))

            return {"success": True, "message": "Backtest completed successfully", "data": backtest_result.to_dict()}
    ...
    def _prepare_data(self, assets: List[Dict[str, Any]], start_date: Optional[str], end_date: Optional[str]) -> pd.DataFrame:
        """
        ...
        dates = pd.date_range(start=start_date or "2023-01-01", end=end_date or "2024-01-01", freq="1H")
    ...
    def _build_fasttrade_config(self, strategy_def: Dict[str, Any], parameters: Dict[str, Any], initial_capital: float, data: pd.DataFrame) -> Dict[str, Any]:
        """
        ...
        if strategy_type == "sma_crossover":
            ...
            config = {"base_balance": initial_capital, "freq": "1H", "comission": 0.001, "datapoints": [{"name": "sma_fast", "transformer": "sma", "args": [fast_period]}, {"name": "sma_slow", "transformer": "sma", "args": [slow_period]}], "enter": [["sma_fast", ">", "sma_slow"]], "exit": [["sma_fast", "<", "sma_slow"]], "trailing_stop_loss": parameters.get("trailingStopLoss", 0.05), "data": data}  # 0.1% commission
            ...
            config = {"base_balance": initial_capital, "freq": "1H", "comission": 0.001, "datapoints": [{"name": "rsi", "transformer": "rsi", "args": [period]}], "enter": [["rsi", "<", oversold]], "exit": [["rsi", ">", overbought]], "trailing_stop_loss": parameters.get("trailingStopLoss", 0.05), "data": data}

        elif strategy_type == "ema_crossover":
            fast_period = parameters.get("fastPeriod", 12)
            slow_period = parameters.get("slowPeriod", 26)

            config = {"base_balance": initial_capital, "freq": "1H", "comission": 0.001, "datapoints": [{"name": "ema_fast", "transformer": "ema", "args": [fast_period]}, {"name": "ema_slow", "transformer": "ema", "args": [slow_period]}], "enter": [["ema_fast", ">", "ema_slow"]], "exit": [["ema_fast", "<", "ema_slow"]], "trailing_stop_loss": parameters.get("trailingStopLoss", 0.05), "data": data}
            ...
            config = {"base_balance": initial_capital, "freq": "1H", "comission": 0.001, "datapoints": [{"name": "macd", "transformer": "macd", "args": [fast_period, slow_period, signal_period]}], "enter": [["macd", ">", 0]], "exit": [["macd", "<", 0]], "trailing_stop_loss": parameters.get("trailingStopLoss", 0.05), "data": data}  # MACD line crosses above zero  # MACD line crosses below zero
            ...
            config = {
                ...
                "freq": "1H",
                "comission": 0.001,
                "datapoints": [{"name": "bb_upper", "transformer": "bbands", "args": [period, std_dev], "column": "upper"}, {"name": "bb_lower", "transformer": "bbands", "args": [period, std_dev], "column": "lower"}],
                "enter": [["close", "<", "bb_lower"]],  # Price below lower band (oversold)
                "exit": [["close", ">", "bb_upper"]],  # Price above upper band (overbought)
            ...
            config = {"base_balance": initial_capital, "freq": "1H", "comission": 0.001, "datapoints": [], "enter": [["close", ">", 0]], "exit": [["close", "<", 0]], "data": data}  # Always enter (buy and hold)  # Never exit
    ...
    def _convert_result(self, ft_result: Dict[str, Any], backtest_id: str) -> BacktestResult:
        ...
        performance = PerformanceMetrics(
            ...
            annualized_return=self._calculate_annualized_return(summary.get("return_perc", 0) / 100, summary.get("total_days", 365)),
    ...
    def _extract_equity_curve(self, df: pd.DataFrame) -> List[EquityPoint]:
        ...
        if "total" in df.columns:
            for date, row in df.iterrows():
                point = EquityPoint(date=str(date), equity=row["total"], returns=0.0, drawdown=0.0, benchmark=None)  # Would need to calculate  # Would need to calculate
...
def main():
    ...
    if len(sys.argv) < 3:
        print(json_response({"success": False, "error": "Usage: python fasttrade_provider.py <command> <json_args>"}))
        sys.exit(1)
