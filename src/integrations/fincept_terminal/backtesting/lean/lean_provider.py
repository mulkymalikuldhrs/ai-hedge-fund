...
class LeanProvider(BacktestingProviderBase):
    ...
    def test_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run([self.lean_cli_path, "--version"], capture_output=True, text=True, timeout=10)
    ...
    def run_backtest(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            command = self._build_cli_command(mode="backtest", project_path=project_path, algorithm="main.py", start_date=start_date, end_date=end_date, cash=initial_capital, data_folder=self.data_path, results_folder=results_folder, parameters=parameters)
            ...
            if process.returncode != 0:
                ...
                return BacktestResult(id=backtest_id, status="failed", performance=self._get_empty_metrics(), trades=[], equity=[], statistics=self._get_empty_statistics(), logs=logs, error=f"Lean CLI exited with code {process.returncode}: {stderr}").to_dict()
            ...
            return BacktestResult(id=backtest_id, status="failed", performance=self._get_empty_metrics(), trades=[], equity=[], statistics=self._get_empty_statistics(), logs=logs, error="Backtest timed out after 1 hour").to_dict()

        except Exception as e:
            self._error(f"Backtest {backtest_id} failed", e)
            logs.append(f"{self._current_timestamp()}: Error: {str(e)}")

            return BacktestResult(id=backtest_id, status="failed", performance=self._get_empty_metrics(), trades=[], equity=[], statistics=self._get_empty_statistics(), logs=logs, error=str(e)).to_dict()
    ...
    def get_historical_data(self, request: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            command = [self.lean_cli_path, "data", "download", "--tickers", ",".join(symbols), "--resolution", timeframe, "--start", self._format_date(start_date), "--end", self._format_date(end_date), "--market", "usa", "--data-folder", self.data_path]

            # Execute download
            result = subprocess.run(command, capture_output=True, text=True, timeout=600)  # 10 minutes
    ...
    def _build_cli_command(self, mode: str, project_path: str, algorithm: str, start_date: Optional[str] = None, end_date: Optional[str] = None, cash: Optional[float] = None, data_folder: Optional[str] = None, results_folder: Optional[str] = None, parameters: Optional[Dict[str, Any]] = None) -> List[str]:
    ...
    def _parse_lean_results(self, backtest_id: str, lean_results: Dict[str, Any], logs: List[str]) -> BacktestResult:
    ...
    def _get_empty_metrics(self) -> PerformanceMetrics:
        ...
        return PerformanceMetrics(total_return=0, annualized_return=0, sharpe_ratio=0, sortino_ratio=0, max_drawdown=0, win_rate=0, loss_rate=0, profit_factor=0, volatility=0, calmar_ratio=0, total_trades=0, winning_trades=0, losing_trades=0, average_win=0, average_loss=0, largest_win=0, largest_loss=0, average_trade_return=0, expectancy=0)

    def _get_empty_statistics(self) -> BacktestStatistics:
        """Get empty statistics"""
        return BacktestStatistics(start_date="", end_date="", initial_capital=0, final_capital=0, total_fees=0, total_slippage=0, total_trades=0, winning_days=0, losing_days=0, average_daily_return=0, best_day=0, worst_day=0, consecutive_wins=0, consecutive_losses=0)
...
def main():
    ...
    if len(sys.argv) < 3:
        print(json_response({"success": False, "error": "Usage: python lean_provider.py <command> <json_args>"}))
        sys.exit(1)
