...
from .core import DerivativeType, OptionType, UnderlyingType, MarketData, PricingResult, ValidationError, ModelValidator, Constants
        ...
        if abs(price_difference) > Constants.EPSILON:
            if price_difference > 0:
                ...
                return ArbitrageOpportunity(
                    ...
                    trade_details={"sell_call": self.call_price, "buy_put": self.put_price, "buy_stock": self.spot_price, "sell_bond": pv_strike, "net_profit": price_difference, "synthetic_call_price": synthetic_call, "actual_call_price": self.call_price},
                ...
                return ArbitrageOpportunity(
                    ...
                    confidence_level=0.95,
                    instruments_involved=["call", "put", "stock", "bond"],
                    trade_details={"buy_call": self.call_price, "sell_put": self.put_price, "sell_stock": self.spot_price, "buy_bond": pv_strike, "net_profit": abs(price_difference), "synthetic_call_price": synthetic_call, "actual_call_price": self.call_price},
                    risk_factors=["early_exercise", "dividend_risk", "interest_rate_risk"],
                    execution_complexity="medium",
                )
        ...
        if abs(price_difference) > Constants.EPSILON:
            if price_difference > 0:
                ...
                return ArbitrageOpportunity(
                    ...
                    confidence_level=0.95,
                    instruments_involved=["call", "put", "stock", "bond"],
                    trade_details={"buy_call": self.call_price, "sell_put": self.put_price, "sell_stock": self.spot_price, "buy_bond": pv_strike, "net_profit": price_difference, "synthetic_put_price": synthetic_put, "actual_put_price": self.put_price},
                    risk_factors=["early_exercise", "dividend_risk", "interest_rate_risk"],
                    execution_complexity="medium",
                )
            else:
                ...
                return ArbitrageOpportunity(
                    ...
                    confidence_level=0.95,
                    instruments_involved=["call", "put", "stock", "bond"],
                    trade_details={"sell_call": self.call_price, "buy_put": self.put_price, "buy_stock": self.spot_price, "sell_bond": pv_strike, "net_profit": abs(price_difference), "synthetic_put_price": synthetic_put, "actual_put_price": self.put_price},
                    risk_factors=["early_exercise", "dividend_risk", "interest_rate_risk"],
                    execution_complexity="medium",
                )
        ...
        if abs(arbitrage_profit) > Constants.EPSILON:
            if arbitrage_profit > 0:
                ...
                return ArbitrageOpportunity(
                    ...
                    instruments_involved=[f"call_{self.K1}", f"call_{self.K2}", f"put_{self.K1}", f"put_{self.K2}"],
                    trade_details={"buy_call_low": self.C1, "sell_call_high": self.C2, "sell_put_low": self.P1, "buy_put_high": self.P2, "box_cost": box_cost, "guaranteed_payoff": guaranteed_payoff, "present_value": present_value_payoff, "arbitrage_profit": arbitrage_profit, "call_spread_cost": self.C1 - self.C2, "put_spread_cost": self.P2 - self.P1},
                ...
                return ArbitrageOpportunity(
                    ...
                    instruments_involved=[f"call_{self.K1}", f"call_{self.K2}", f"put_{self.K1}", f"put_{self.K2}"],
                    trade_details={"sell_call_low": self.C1, "buy_call_high": self.C2, "buy_put_low": self.P1, "sell_put_high": self.P2, "box_revenue": -box_cost, "guaranteed_payout": -guaranteed_payoff, "arbitrage_profit": abs(arbitrage_profit), "call_spread_revenue": -(self.C1 - self.C2), "put_spread_revenue": -(self.P2 - self.P1)},
                    risk_factors=["execution_risk", "bid_ask_spread"],
                    execution_complexity="high",
                )
        ...
        if abs(price_difference) > Constants.EPSILON:
            ...
            if price_difference > 0:
                ...
                return ArbitrageOpportunity(
                    ...
                    trade_details={"sell_forward": self.forward_price, "buy_underlying": self.spot_price, "borrow_funds": self.spot_price, "theoretical_forward": theoretical_forward, "price_difference": price_difference, "carry_rate": carry_rate, "dividend_yield": self.dividend_yield, "storage_cost": self.storage_cost, "convenience_yield": self.convenience_yield},
                ...
                return ArbitrageOpportunity(
                    ...
                    confidence_level=0.90,
                    instruments_involved=["forward", "underlying", "bond"],
                    trade_details={"buy_forward": self.forward_price, "sell_underlying": self.spot_price, "invest_proceeds": self.spot_price, "theoretical_forward": theoretical_forward, "price_difference": price_difference, "carry_rate": carry_rate, "dividend_yield": self.dividend_yield, "storage_cost": self.storage_cost, "convenience_yield": self.convenience_yield},
                    risk_factors=["storage_costs", "convenience_yield", "dividend_changes", "interest_rate_risk"],
                    execution_complexity="medium",
                )
        ...
        if vol_spread_pct > self.confidence_threshold:
            ...
            if vol_difference > 0:
                ...
                return ArbitrageOpportunity(
                    ...
                    trade_details={"sell_option": implied_vol_price, "market_vol": self.market_vol, "implied_vol": self.implied_vol, "vol_difference": vol_difference, "price_difference": price_difference, "strategy": "sell_option_delta_hedge", "vol_spread_pct": vol_spread_pct, "market_vol_price": market_vol_price, "implied_vol_price": implied_vol_price},
                ...
                return ArbitrageOpportunity(
                    ...
                    instruments_involved=["option", "underlying"],
                    trade_details={"buy_option": implied_vol_price, "market_vol": self.market_vol, "implied_vol": self.implied_vol, "vol_difference": vol_difference, "price_difference": price_difference, "strategy": "buy_option_delta_hedge", "vol_spread_pct": vol_spread_pct, "market_vol_price": market_vol_price, "implied_vol_price": implied_vol_price},
                    risk_factors=["gamma_risk", "vol_risk", "time_decay", "model_risk"],
                    execution_complexity="high",
                )
        ...
        if abs(price_difference) > Constants.EPSILON:
            if price_difference > 0:
                ...
                return ArbitrageOpportunity(
                    ...
                    trade_details={"sell_far_option": self.far_option_price, "buy_near_option": self.near_option_price, "calendar_spread_cost": time_decay_value, "theoretical_time_decay": theoretical_time_decay, "arbitrage_profit": price_difference, "near_time": self.near_time, "far_time": self.far_time},
                ...
                return ArbitrageOpportunity(
                    ...
                    confidence_level=0.80,
                    instruments_involved=[f"{self.option_type.value}_near", f"{self.option_type.value}_far"],
                    trade_details={"buy_far_option": self.far_option_price, "sell_near_option": self.near_option_price, "calendar_spread_cost": time_decay_value, "theoretical_time_decay": theoretical_time_decay, "arbitrage_profit": abs(price_difference), "near_time": self.near_time, "far_time": self.far_time},
                    risk_factors=["volatility_risk", "time_decay", "pin_risk"],
                    execution_complexity="medium",
                )
...
class SyntheticInstrumentBuilder:
    ...
    @staticmethod
    def synthetic_call(put_price: float, spot_price: float, strike_price: float, risk_free_rate: float, time_to_expiry: float, dividend_yield: float = 0.0) -> SyntheticInstrument:
        ...
        return SyntheticInstrument(target_instrument="call", synthetic_components=[{"instrument": "put", "position": "long", "price": put_price, "quantity": 1}, {"instrument": "stock", "position": "long", "price": spot_price, "quantity": 1}, {"instrument": "bond", "position": "short", "price": pv_strike, "quantity": 1}], cost_comparison=synthetic_call_price, replication_accuracy=0.99)

    @staticmethod
    def synthetic_put(call_price: float, spot_price: float, strike_price: float, risk_free_rate: float, time_to_expiry: float, dividend_yield: float = 0.0) -> SyntheticInstrument:
        ...
        return SyntheticInstrument(target_instrument="put", synthetic_components=[{"instrument": "call", "position": "long", "price": call_price, "quantity": 1}, {"instrument": "bond", "position": "long", "price": pv_strike, "quantity": 1}, {"instrument": "stock", "position": "short", "price": spot_price, "quantity": 1}], cost_comparison=synthetic_put_price, replication_accuracy=0.99)

    @staticmethod
    def synthetic_stock(call_price: float, put_price: float, strike_price: float, risk_free_rate: float, time_to_expiry: float, dividend_yield: float = 0.0) -> SyntheticInstrument:
        """Create synthetic stock using options: S = C - P + K*e^(-rT)"""
        pv_strike = strike_price * np.exp(-risk_free_rate * time_to_expiry)

        synthetic_stock_price = call_price - put_price + pv_strike

        return SyntheticInstrument(target_instrument="stock", synthetic_components=[{"instrument": "call", "position": "long", "price": call_price, "quantity": 1}, {"instrument": "put", "position": "short", "price": put_price, "quantity": 1}, {"instrument": "bond", "position": "long", "price": pv_strike, "quantity": 1}], cost_comparison=synthetic_stock_price, replication_accuracy=0.95)

    @staticmethod
    def synthetic_forward(spot_price: float, risk_free_rate: float, time_to_expiry: float, dividend_yield: float = 0.0) -> SyntheticInstrument:
        ...
        return SyntheticInstrument(target_instrument="forward", synthetic_components=[{"instrument": "stock", "position": "long", "price": spot_price, "quantity": 1}, {"instrument": "bond", "position": "short", "price": spot_price - pv_dividends, "quantity": 1}], cost_comparison=forward_price, replication_accuracy=0.99)

    @staticmethod
    def synthetic_bond(strike_price: float, call_price: float, put_price: float, spot_price: float, dividend_yield: float = 0.0) -> SyntheticInstrument:
        ...
        return SyntheticInstrument(target_instrument="bond", synthetic_components=[{"instrument": "call", "position": "long", "price": call_price, "quantity": 1}, {"instrument": "put", "position": "short", "price": put_price, "quantity": 1}, {"instrument": "stock", "position": "long", "price": spot_price, "quantity": 1}], cost_comparison=cost_to_replicate, replication_accuracy=0.98)
    ...
    def generate_execution_plan(self, opportunity: ArbitrageOpportunity) -> Dict[str, Any]:
        ...
        if opportunity.arbitrage_type == ArbitrageType.CONVERSION:
            ...
            if opportunity.direction == ArbitrageDirection.BUY_CHEAP_SELL_EXPENSIVE:
                execution_steps = [
                    {"step": 1, "action": "Buy call (low strike)", "details": opportunity.trade_details.get("buy_call_low")},
                    {"step": 2, "action": "Sell call (high strike)", "details": opportunity.trade_details.get("sell_call_high")},
                    {"step": 3, "action": "Sell put (low strike)", "details": opportunity.trade_details.get("sell_put_low")},
                    {"step": 4, "action": "Buy put (high strike)", "details": opportunity.trade_details.get("buy_put_high")},
                ]
            else:
                execution_steps = [
                    {"step": 1, "action": "Sell call (low strike)", "details": opportunity.trade_details.get("sell_call_low")},
                    {"step": 2, "action": "Buy call (high strike)", "details": opportunity.trade_details.get("buy_call_high")},
                    {"step": 3, "action": "Buy put (low strike)", "details": opportunity.trade_details.get("buy_put_low")},
                    {"step": 4, "action": "Sell put (high strike)", "details": opportunity.trade_details.get("sell_put_high")},
                ]
...
# Export main classes
__all__ = ["ArbitrageType", "ArbitrageDirection", "ArbitrageOpportunity", "SyntheticInstrument", "ConversionStrategy", "ReversalStrategy", "BoxSpreadStrategy", "CarryArbitrageDetector", "VolatilityArbitrageDetector", "CalendarSpreadArbitrage", "SyntheticInstrumentBuilder", "ArbitrageScanner"]
