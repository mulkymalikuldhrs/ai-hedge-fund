...
class UnifiedBacktester:
            ...
            for ticker, df in data.items():
                ...
                for name, strategy in self.strategies.items():
                    signal = strategy.generate_signal(
                        ticker, high, low, close, volume, current_idx
                    )
                    ...
                    if signal:
                        ...
                        self._execute_signal(
                            signal, ticker, current_price, date, capital, positions
                        )
        ...
        for ticker, pos in list(positions.items()):
            if pos['quantity'] > 0:
                self._close_position(
                    pos, data[ticker]['close'].iloc[-1], 
                    data[ticker].index[-1], capital, positions
                )
    ...
    def run_walk_forward(
        ...
        test_period: int = 21,    # Days
        strategy_name: str = "WalkForward"
    ) -> List[BacktestResult]:
        """
    ...
    def _execute_signal(
        ...
        if action == 'BUY' and ticker not in positions:
            ...
            positions[ticker] = {
                ...
                'entry_date': date,
                'direction': 'LONG',
                'source': signal.get('source', SignalSource.COMBINED),
            ...
            self.trades.append(Trade(
                ...
                direction='LONG',
                pnl=-fee,
                pnl_pct=-fee / position_value * 100,
                commission=fee,
                source=signal.get('source', SignalSource.COMBINED),
            ...
            self._close_position(
                positions[ticker], price, date, capital, positions
            )
    ...
    def _calculate_results(
        ...
        trade_df = pd.DataFrame([
            {'date': t.exit_date, 'pnl': t.pnl} for t in trades
        ])
        if len(trade_df) > 0:
        ...
        trade_analysis = {
            ...
            for source, pnls in trade_by_source.items()
        }
...
if __name__ == "__main__":
    ...
    def wyckoff_signal(high, low, close, volume):
        ...
        return {
            'action': strategy.calculate(high, low, close, volume).action,
            'confidence': strategy.calculate(high, low, close, volume).confidence,
            'stop_loss': strategy.calculate(high, low, close, volume).stop_loss,
            'take_profit': strategy.calculate(high, low, close, volume).take_profit,
